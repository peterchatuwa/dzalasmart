...
export const MAJOR_TRADING_CENTRES = [
  ...
  { name: "Mchinji Boma", district: "Mchinji", lat: -13.8, lon: 32.88 },
  { name: "Mzuzu City", district: "Mzimba", lat: -11.45, lon: 34.02 },
  { name: "Blantyre City", district: "Blantyre", lat: -15.79, lon: 35.0 },
  ...
  { name: "Nkhotakota Boma", district: "Nkhotakota", lat: -12.92, lon: 34.3 },
  { name: "Dedza Boma", district: "Dedza", lat: -14.38, lon: 34.33 },
  { name: "Ntcheu Boma", district: "Ntcheu", lat: -14.82, lon: 34.63 },
  { name: "Mulanje Boma", district: "Mulanje", lat: -16.03, lon: 35.5 },
...
export function tradingCentreSlug(name, district) {
  return `tc-${districtSlug(district)}-${String(name || "")
    .trim()
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, "-")
    .replace(/^-|-$/g, "")}`;
...
export async function listLocationTree(db, filters = {}) {
  ...
  if (filters.district) {
    ...
    return roots.flatMap((region) =>
      region.children.filter((node) => node.district === district || node.name === district)
    );
  }
  if (filters.region) {
